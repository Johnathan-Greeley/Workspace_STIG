...
def V220524(stig, scan_ctx):
    ...
    if re.search(r'login block-for 900 attempts 3 within 120', result):
        stig.status = "NF"
...
def V220548(stig, scan_ctx):
    ...
    # Check if the res.clean contains the required configuration
    if "Logging to" in res.clean and re.search(r"(debugging|critical|warnings|notifications|informational)", result):
        stig.status = "NF"
...
def V220554(stig, scan_ctx):
    ...
    if "configured," in res.clean and "authenticated" in result:
...
def V220664(stig, scan_ctx):
    ...
    if "rapid" in res.clean or "mst" in result:
        stig.status = "NF"
...
def V220665(stig, scan_ctx):
    ...
    if "enable" in res.clean or "aggressive" in result:
        stig.status = "NF"
...
def V221006(stig, scan_ctx):
    ...
    if "line aux" not in res.clean or "no exec" not in result:
...
# def V220677(device_type, device_name):
    ...
    # result = some_undefined_variable + res.clean
