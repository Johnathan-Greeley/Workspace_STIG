...
class Stig:
    """
    Lightweight data‑object for one vulnerability result.
...
def exec_command(command, device_name):
    ...
    if output is None:
        ...
        result = handle_errors(res.clean, command, device_name)
...
def V237756(stig, scan_ctx):
    result = scan_ctx.exec("show ipv6 interface").clean
    line_count = len(res.clean.strip().split('\n'))
...
def V220682(stig, scan_ctx):
    ...
    result = scan_ctx.exec("show run | i loopguard").clean
...
def V220693(stig, scan_ctx):
    ...
    result = res.clean + "\r" + exec_command(command, device_name)
    if (
...
def V215856(stig, scan_ctx):
    ...
    result = res.clean + exec_command(command, device_name)
    stig.finding = res.rendered
