...
def crt_handle_errors(result, command, device_name):
    ...
    Args:
    - result (str): The res.clean from the command execution.
...
def V220532(stig, scan_ctx):
    ...
    if "file privilege 15" in res.clean:
        stig.status = "NF"
...
def V220657(stig, scan_ctx):
    ...
    if "loopguard default" in res.clean:
        stig.status = "NF"
...
def V220668(stig, scan_ctx):
    ...
    if "does not exist" in res.clean:
        stig.status = "NF"
...
def V220669(stig, scan_ctx):
    ...
    if re.search("does not exist", res.clean):
        stig.status = "NF"
