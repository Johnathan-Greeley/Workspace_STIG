...
def V237756(stig, ctx):
    result = ctx.exec("show ipv6 interface").clean
    line_count = len(result.strip().split('\n'))
...
def V220680(stig, ctx):
    ...
    root_ports = list(set([
        line.split(" ")[0]
        for line in ctx.exec("show spanning-tree brief | i Root.FWD").clean.splitlines()
    ...
    trunk_ports = list(set([
        line.split(" ")[0]
        for line in ctx.exec("show int trunk | i trunking | exc not-trunking").clean.splitlines()
...
def V220687(stig, ctx):

    command = ctx.exec("show run | sec ^interface|^port-profile | ex .access.vlan.666|ip|mtu|trunk.(native|allowed)").clean
...
def create_stig_list_from_host(device_name, checklist_file):
    ...
    cache = Commandcache()
    ctx   = ScanContext(device_name, send_command, cache)
    ...
    for idx, (orig_vuln, (func_name, severity)) in enumerate(vuln_info.items(), 1):
        try:
            ...
            vul_func(stig, ctx)           # <── new slim signature
