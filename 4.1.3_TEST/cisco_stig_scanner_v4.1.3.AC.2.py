...
def V220554(stig, scan_ctx):
    ...
    # Execute command
    result = exec_command(command, device_name)
    stig.finding = result  # Save the result to stig.finding
...
def V220673(stig, scan_ctx):
    ...
    trunk_output = exec_command(trunk_cmd, device_name)
    ...
    check_output = exec_command(check_cmd, device_name)
...
def V221021(stig, scan_ctx):
    ...
    stig.finding = result  # Store the result as the finding
...
def V220487(stig, scan_ctx):
    ...
    stig.finding = result  # Store the sanitized result as finding
    return check    
...
def V216655(stig, scan_ctx):
    ...
    if result.find("RT1") > -1:
        ...
        result = exec_command(command, device_name)
        stig.finding = stig.finding + result
        if (
